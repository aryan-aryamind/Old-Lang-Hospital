...
class GoogleCloudTTS:
    ...
    def generate_speech(
        ...
        try:
            ...
            if voice_name:
                # When using a specific voice name, set it and ensure language code aligns
                voice_params.name = voice_name
                if not language_code and '-' in voice_name:
                    parts = voice_name.split('-')
                    if len(parts) >= 2:
                        language_code = parts[0] + '-' + parts[1]
                voice_params.language_code = language_code or self.default_voice['language_code']
            elif language_code:
